...
import { IAddWindowProperties, IWindowProperties, WindowState } from '../interfaces/windows'
...
export const WindowsContext: React.Context<IWindowsContext> = createContext<IWindowsContext>({
    windowProperties: [],
...
const WindowsContextProvider = (props: IWindowsContextProviderProps) => {
    ...
    const onMinimizeAllButtonClicked = () => {
        setWindowProperties([...windowProperties.map(x => {
            return {
                ...x,
                selected: false,
    ...
    const onWindowStateChanged = (windowId: string, newState: WindowState) => {
        ...
        if (targetWindowIndex !== -1) {
            ...
            let newWindow = {
                ...
                state: newState,
                selected: newState === WindowState.Minimised ? false : newWindows[targetWindowIndex].selected
            }
    ...
    const onTaskbarItemClicked = (windowId: string) => {
        ...
        if (targetWindowIndex !== -1) {
            ...
            if (existingWindow.state === WindowState.Minimised) {
                existingWindow.state = existingWindow.previousState ?? WindowState.Normal
    ...
    const onWindowSelected = (windowId: string) => {
        ...
        if (existingWindow && existingWindow.selected
        ...
        const existingWindowIndex = newWindowProperties.findIndex(wp => wp.id === windowId)
    ...
    const removeWindow = (windowId: string) => setWindowProperties(x => [...x.filter(x => x.id !== windowId)])

			...
			<div className="file-browser-grid-view__content">
				{entities.map((e) => {
					...
					return (
						...
						>
							<div
								className={`file-browser-grid-view__content__row__cell__name ${NO_SELECT_CLASS}`}
							>
								{icon}
								<span>{e.fullName}</span>
							</div>
							<div
								className={`file-browser-grid-view__content__row__cell__date ${NO_SELECT_CLASS}`}
							>
								<span>{formattedDate}</span>
							</div>
							<div
								className={`file-browser-grid-view__content__row__cell__type ${NO_SELECT_CLASS}`}
							>
								<span>{fileTypeDescriptor}</span>
							</div>
							<div
								className={`file-browser-grid-view__content__row__cell__size ${NO_SELECT_CLASS}`}
							>
								<span>{0}</span>
							</div>
						</div>
